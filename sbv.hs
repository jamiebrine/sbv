...
-- Checks that a character is a letter and not the O symbol (reserved for identity)
isVariableChar :: Char -> Bool
isVariableChar c = isLetter c && (c /= 'O') 
...
-- Reorders par and copar structures into a predefined normal form (Seq, Par, Copar, Var, Not Var) 
reorder :: Term -> Term
...
sortTerms s p c v n []               = reverse s ++ reverse p ++ reverse c ++ sortVars v ++ sortVars n
...
normalise :: Term -> Term
normalise t = reorder (doubleNegative (deMorgan (associate (removeId t))))